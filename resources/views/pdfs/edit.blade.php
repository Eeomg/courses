                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('pdfs.update', $pdf->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="title" class="form-control form-control-lg shadow-sm" id="title" value="{{ old('title',$pdf->title) }}" required>
                            </div>
                            <div class="mb-3">
                                <label for="description" class="form-label fw-bold">Description</label>
                                <textarea name="description" class="form-control form-control-lg shadow-sm" id="description" rows="4">{{ old('description',$pdf->description) }}</textarea>
                            </div>
                                ...
                                <div class="input-group shadow-sm">
                                    ...
                                    <select name="video_id" id="video_id" class="form-select form-control form-select-lg border-primary" required>
                                        ...
                                        @foreach($videos as $video)
                                            <option value="{{ $video->id }}" {{ $pdf->video_id == $video->id ? 'selected' : '' }}>{{ $video->title }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="d-grid gap-2 mt-4">
                                ...
                                <a href="{{ route('courses.show', $pdf->course_id) }}" class="btn btn-secondary btn-lg">Cancel</a>
                            </div>
                        </form>
                    </div>
                </div>
